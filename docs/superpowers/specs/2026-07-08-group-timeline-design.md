...
- `src/hooks/useGroup.ts` — hook central instancié dans `App.tsx` :
  ...
  - publication débouncée (~2 s) à chaque changement de favoris/pseudo/présence, + `online` + `visibilitychange` ; réception → déchiffrement → garde le plus récent (`updatedAt`) → cache ; si l'état reçu porte `left: true`, le pubkey est retiré du cache au lieu d'être fusionné (l'ami disparaît de `others` et ses ids sortent de `friendsByEvent`) ;
...
- `EventCard` — props optionnelles `friends` (`{name, here}[]`) et `presence` (`{here, onToggle}`, fournie par `ProgramTab` quand on est en groupe et que l'événement est en cours) ; pastilles et bouton sous le lieu, poing intouché.
- `TimelineTab` — bouton « Voir les favoris de mes amis » (switch, actif par défaut, persisté dans `pdj26-show-friends-favorites`) visible dès qu'un groupe existe. Quand actif, la liste affichée fusionne mes favoris et les événements favoris de `others` (union des ids) ; chaque événement montre les pastilles des amis qui l'ont favori. Les événements qui ne sont pas dans mes propres favoris sont visuellement distingués (`.tl-item-friend`, contour en pointillés) et leur poing reste cliquable pour se les approprier.
